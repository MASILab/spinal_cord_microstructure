...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_norm/intra_extra_rest_watson_csf'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + '_watson_csf.json'
            signal_file_name = cyn_key + '_' + gsn_key + '_watson' + '_csf_signal.txt'
            ...
            watson_dispersed_intra = SD1WatsonDistributed(models=[cylinder, gaussian])
            multi_compat_model = MultiCompartmentModel(models=[watson_dispersed_intra, csf_ball])
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                # Fit Cylinder/Intra-cellular to Watson Distirbutions
                watson_dispersed_intra = SD1WatsonDistributed(models=[cylinder, gaussian])
                multi_compat_model = MultiCompartmentModel(models=[watson_dispersed_intra, sphere, csf_ball])
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_watson_csf.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_watson' + '_csf_signal.txt'
