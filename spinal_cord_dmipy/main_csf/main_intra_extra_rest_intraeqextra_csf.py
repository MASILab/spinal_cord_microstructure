...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_norm/intra_extra_rest_intraextra_equal_csf'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + 'intra_extra_eq' + '_csf.json'
            signal_file_name = cyn_key + '_' + gsn_key + 'intra_extra_eq' + '_csf_signal.txt'
            ...
            # TODO We will be setting all parallel diffusivity equal to each other for intra and extra.
            par_list = []
            for each_para_name in multi_compat_model.parameter_names:
                # Last three characters of parameter
                diffusivity_type = each_para_name[-3:]
                if diffusivity_type == 'par':
                    par_list.append(each_para_name)
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + 'intra_extra_eq' + '_csf.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + 'intra_extra_eq' + '_csf_signal.txt'
