...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_norm/intra_extra_rest_extra_greater_csf'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + '_extra_greater' + '_csf.json'
            signal_file_name = cyn_key + '_' + gsn_key + '_extra_greater' + '_csf_signal.txt'
            ...
            if len(intra_par_list) >= 1 and len(extra_par_list) >= 1:
                multi_compat_model.set_fractional_parameter(intra_par_list[0], extra_par_list[0])
            # if len(par_list) == 2:
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_extra_greater' + '_csf.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_extra_greater' + '_csf_signal.txt'
