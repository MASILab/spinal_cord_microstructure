...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_norm/intra_extra_rest_intra_greater_csf'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + '_intra_greater' + '_csf.json'
            signal_file_name = cyn_key + '_' + gsn_key + '_intra_greater' + '_csf_signal.txt'
            ...
            # TODO We will be setting all parallel diffusivity for intra to be greater than extra.
            intra_par_list = []
            for each_para_name in multi_compat_model.parameter_names:
                # Last three characters of parameter
                diffusivity_type = each_para_name[-3:]
                model_type = each_para_name[0]
                if diffusivity_type == 'par' and model_type=='C':
                    intra_par_list.append(each_para_name)
            ...
            for each_para_name in multi_compat_model.parameter_names:
                ...
                diffusivity_type = each_para_name[-3:]
                model_type = each_para_name[0]
                if diffusivity_type == 'par' or diffusivity_type == 'iso':
                    if model_type == 'G':
                        extra_par_list.append(each_para_name)

            if len(intra_par_list)>=1 and len(extra_par_list)>=1:
                multi_compat_model.set_fractional_parameter(extra_par_list[0], intra_par_list[0])
            #if len(par_list) == 2:
            #    multi_compat_model.set_equal_parameter(par_list[0], par_list[1])
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_intra_greater' + '_csf.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_intra_greater' + '_csf_signal.txt'
