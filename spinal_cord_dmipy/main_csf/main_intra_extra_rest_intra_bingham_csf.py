...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_norm/intra_extra_rest_bingham_csf'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + '_bingham_csf.json'
            signal_file_name = cyn_key + '_' + gsn_key + '_bingham' + '_csf_signal.txt'
            ...
            # Fit Cylinder/Intra-cellular to Bingham Distirbutions
            bingham_dispersed_intra = SD2BinghamDistributed(models=[cylinder, gaussian])
            multi_compat_model = MultiCompartmentModel(models=[bingham_dispersed_intra, csf_ball])
            ...
            # For Bingham distribution the scenario changes as the ball will always be G1Ball as
            # for other parameters Bingham gets added in
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                # Fit Cylinder/Intra-cellular to Bingham Distirbutions
                bingham_dispersed_intra = SD2BinghamDistributed(models=[cylinder, gaussian])
                multi_compat_model = MultiCompartmentModel(models=[bingham_dispersed_intra, sphere, csf_ball])
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_bingham_csf.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_bingham' + '_csf_signal.txt'
