...
def main():
    ...
    save_data_path = r'/nfs/masi/nathv/spinal_cord_data_2020/results_2/intra_extra_rest_tort'
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            ...
            # File name
            model_file_name = cyn_key + '_' + gsn_key + '_tort' + '.json'
            signal_file_name = cyn_key + '_' + gsn_key + '_tort' + '_signal.txt'
            ...
            for each_para_name in bundle_model.parameter_names:
                ...
                if diffusivity_type == 'perp':
                    perp_list.append(each_para_name)
                    #multi_compat_model.set_fixed_parameter(each_para_name, 1.7e-9)
    ...
    for cyn_key, cyn_val in cylinder_dict.items():
        for gsn_key, gsn_val in gaussian_dict.items():
            for sph_key, sph_val in sphere_dict.items():
                ...
                if len(perp_list)>=1 and len(par_list)>=1:
                    multi_compat_model.set_tortuous_parameter(perp_list[0],
                                                              par_list[0],
                                                              'partial_volume_0',
                                                              'partial_volume_1'
                                                              )
                ...
                model_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_tort' + '.json'
                signal_file_name = cyn_key + '_' + gsn_key + '_' + sph_key + '_tort' + '_signal.txt'
